...
class TestModulation:
    ...
    def test_modulation_output_structure(self):
        ...
        # For double=True, output should contain shift/scale tuples
        assert isinstance(output, tuple) or isinstance(output, torch.Tensor)
        # gate can be tensor or None depending on implementation
    ...
    def test_modulation_single_vs_double(self):
        ...
        out_single, _ = mod_single(vec)
        out_double, _ = mod_double(vec)

        # Double should produce more modulation values
        # (shift/scale for both img and txt in double blocks)
...
class TestDoubleStreamBlock:
    ...
    def test_double_block_output_shapes(self, block, params):
        ...
        # Create modulation (shift, scale, gate for img and txt)
        mod_img = (
            torch.zeros(batch, params.hidden_size),
            torch.ones(batch, params.hidden_size),
            torch.ones(batch, params.hidden_size),
        )
        mod_txt = (
            torch.zeros(batch, params.hidden_size),
...
class TestSingleStreamBlock:
    ...
    def test_single_block_instantiation(self, block):
        ...
        assert hasattr(block, "attn") or hasattr(block, "linear1")
...
class TestLastLayer:
    ...
    def test_last_layer_instantiation(self):
        ...
        hidden_size = 4096
        patch_size = 1  # After unpatchify, this might be 1
        out_channels = 128  # Back to patchified latent channels

        layer = LastLayer(hidden_size, patch_size, out_channels)
    ...
    def test_last_layer_output_shape(self):
        ...
        mod = torch.randn(batch, hidden_size)  # Final modulation
