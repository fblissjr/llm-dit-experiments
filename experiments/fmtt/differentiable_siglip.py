"""Differentiable SigLIP2 wrapper for FMTT reward computation.

The key difference from experiments/metrics/siglip_score.py is that
the image processing path is gradient-enabled, allowing backprop
through the reward to the input latents.
...
class DifferentiableSigLIP:
    ...
    Advantages over CLIP:
        ...
        - Better zero-shot (+2-3% on ImageNet)
        - Better retrieval (56% vs 45% R@1 on COCO)
    ...
    def compute_reward(
        ...
        Args:
            ...
            return_similarity: If True, return raw cosine similarity;
                              if False, could apply reward shaping (not implemented)
...
def test_gradient_flow():
    ...
    if image.grad is not None and not image.grad.isnan().any() and not image.grad.isinf().any():
