...
def main():
    ...
    for name, r in results.items():
        print(
            f"{name:<20} {r['num_tokens']:<8} {r['dim_4_mean']:<12.1f} {r['dim_4_per_token']:<12.1f} {r['dim_396_mean']:<12.1f} {r['dim_396_per_token']:<12.1f}"
        )
    ...
    print(
        f"   'A cat' ({short['num_tokens']} tokens) vs repeated ({repeated['num_tokens']} tokens)"
    )
    print(
        f"   Dim 4 mean:  {short['dim_4_mean']:.1f} vs {repeated['dim_4_mean']:.1f} (ratio: {repeated['dim_4_mean'] / short['dim_4_mean']:.2f}x)"
    )
    ...
    print(
        f"   'A cat' ({short['num_tokens']} tokens) vs expanded ({expanded['num_tokens']} tokens)"
    )
    print(
        f"   Dim 4 mean:  {short['dim_4_mean']:.1f} vs {expanded['dim_4_mean']:.1f} (ratio: {expanded['dim_4_mean'] / short['dim_4_mean']:.2f}x)"
    )
