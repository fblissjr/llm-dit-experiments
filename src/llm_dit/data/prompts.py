"""
LTX-2 Official Prompts and Experiment Datasets.
...
Key format requirements (from official guide):
...
Usage:
    from llm_dit.data import get_official_prompts, get_all_prompts

    # Get all official prompts
    prompts = get_official_prompts()

    # Quick mode for development
    prompts = get_all_prompts(quick=True)

    # Get structured prompts for format ablation
    formats = get_structured_prompts()
"""
...
def get_official_prompts(quick: bool = False) -> dict[str, str]:
    ...
    Args:
        quick: If True, return only 3 diverse examples
...
def get_category_prompts(quick: bool = False) -> dict[str, str]:
    ...
    Args:
        quick: If True, return only 2 diverse examples
...
def get_all_prompts(quick: bool = False) -> dict[str, str]:
    ...
    Args:
        quick: If True, return reduced set for faster iteration
...
def get_structured_prompts(quick: bool = False) -> dict[str, str]:
    ...
    Args:
        quick: If True, return only prose and JSON

    Returns:
        Dictionary of format_name -> prompt_text
    """
    if quick:
...
def validate_prompts() -> bool:
    ...
    Returns:
        True if all prompts valid, False otherwise
    """
    issues = []
