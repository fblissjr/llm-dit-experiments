"""
LTX-2 VAE Tiling System.

Last Updated: 2026-01-18

Tiling utilities for memory-efficient VAE encoding/decoding of large videos.

Ported from: ltx_core.model.video_vae.tiling
...
class TilingConfig:
    ...
    def default(cls) -> "TilingConfig":
        """Default tiling configuration for LTX-2."""
        return cls(
...
class DimensionIntervals:
    ...
    Ramps are regions where the mask value is interpolated for blending.
    """
    starts: List[int]
    ends: List[int]
    left_ramps: List[int]
...
def default_split_operation(length: int) -> DimensionIntervals:
    """Default split: single interval covering entire dimension."""
...
def default_mapping_operation(
    ...
    """Default mapping: full slice, no mask."""
...
class Tile(NamedTuple):
    """
    Represents a single tile for tiled processing.

    Attributes:
        in_coords: Tuple of slices specifying where to cut the tile from INPUT tensor.
        out_coords: Tuple of slices specifying where this tile's OUTPUT goes in result.
        masks_1d: Per-dimension masks in OUTPUT units for blending.
    """
...
def create_tiles_from_intervals_and_mappers(
    ...
    """Create tiles from intervals and mapping operations."""
...
def create_tiles(
    ...
    """
    Create tiles for processing a latent tensor.

    Args:
        latent_shape: Shape of the latent tensor.
        splitters: Split operations for each dimension.
        mappers: Mapping operations for each dimension.
    ...
    if len(splitters) != len(latent_shape):
        raise ValueError(
            f"Number of splitters must equal number of dimensions in latent shape, "
            f"got {len(splitters)} and {len(latent_shape)}"
        )
    if len(mappers) != len(latent_shape):
        raise ValueError(
            f"Number of mappers must equal number of dimensions in latent shape, "
